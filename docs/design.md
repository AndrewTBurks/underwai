...
**Stale UX reference behavior.** One reference, not a lib mandate: a `stale` node shows its previous output (from the prior `resolved` or `streaming` state) with a "re-deriving" indicator. The exact rendering is up to the consumer; the lib does not enforce it. The reference is documented here so renderers have a starting point and so consumers can know what to expect from a "good" UX. The lib's contract is the state machine, not the UI.
