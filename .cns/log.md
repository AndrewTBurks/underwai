...
- 2026-06-12: TASK-50 done — extracted the examples demo model and runtime controller. Demo/ScenarioKind moved to demo-types.ts; runtime state, subscriptions, root writes, human writes, paused detection, and event capture moved to useDemoRuntime.ts; ExampleShell is layout/navigation glue. Workflow definitions, RenderedPanel, and ScenarioSurface now import Demo from demo-types instead of ExampleShell. Verified examples typecheck/test/build, root pnpm build/test (116/116), and CNS health gate. Sharded as DEC-EXAMPLES-012 through DEC-EXAMPLES-015.
- 2026-06-12: Ran `/impeccable critique` + `/impeccable audit` on the live examples page with PRODUCT.md loaded. Verified the human-form enum now renders as a select, reproduced the send-values-to-runtime no-op, and converted findings into TASK-64 through TASK-68 in intent.md.
