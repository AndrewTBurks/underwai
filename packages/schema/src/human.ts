...
export type HumanSchema<T extends ZodTypeAny> = ZodType<
  T["_output"],
  T["_def"],
  T["_input"]
> & {
...
function cloneWithMode<T extends ZodTypeAny>(
  schema: T,
  mode: HumanMode,
): HumanSchema<T> {
  const Ctor = Object.getPrototypeOf(schema).constructor as new (
    def: object,
  ) => T
