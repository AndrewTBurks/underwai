...
function makeState(nodes: Record<string, Node>, edges: Array<{ from: string; to: string }> = []): WorkflowState {
  const es: Array<{ from: import("./keys.js").NodeKey; to: import("./keys.js").NodeKey }> = edges.map((e) => ({ from: NodeKey(e.from), to: NodeKey(e.to) }))
...
describe("getHumanInputDisplay()", () => {
  ...
  it("returns 'from_node' for a node whose input came from an upstream node", () => {
    ...
    const state = makeState(
      { "root.upstream": upstream, root: downstream },
